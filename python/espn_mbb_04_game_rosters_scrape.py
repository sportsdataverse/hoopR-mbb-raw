...
"""Scrape ESPN MBB per-game rosters.

Output: ``mbb/game_rosters/json/{game_id}.json`` -- raw ESPN response.
The downstream R parser in ``hoopR-mbb-data`` reads these JSONs and
navigates ``boxscore.players`` to build the per-game tidy roster frame
(one row per athlete-team-game).

Endpoint: ``site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/summary``
This is the same comprehensive game-summary endpoint espn_mbb_02_pbp_scrape.py
...
``sportsdataverse.nba.espn_mbb_game_rosters`` because that SDK helper
...
(``mbb/schedules/parquet/mbb_schedule_{year}.parquet``). If the parquet
is missing, falls back to a fresh ``sdv.mbb.espn_mbb_schedule`` call.
"""
...
# Bypass the broken espn_mbb_game_rosters helper (see module docstring);
...
logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_mbb_raw_game_rosters_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "mbb/game_rosters/json"
PATH_TO_SCHEDULES = "mbb/schedules/parquet"
SUMMARY_URL = "http://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/summary?event={game_id}"
MAX_THREADS = 8


def fetch_game_ids_for_season(season: int) -> list[int]:
    """Pull every completed game id ESPN exposes for a given MBB season.
    ...
    fresh ``espn_mbb_schedule`` call when the parquet is missing.
    """
    schedule_path = Path(f"{PATH_TO_SCHEDULES}/mbb_schedule_{season}.parquet")
    ...
    logger.warning(
        f"No schedule parquet at {schedule_path}; falling back to espn_mbb_schedule()"
    )
    sched = sdv.mbb.espn_mbb_schedule(season=season)
...
def download_game_rosters_batch(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for fut in tqdm(
            ...
            total=len(futs),
            desc=f"MBB game rosters {season}",
        ):
            fut.result()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of MBB game-roster scrape (YYYY), e.g. 2025",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of MBB game-roster scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",

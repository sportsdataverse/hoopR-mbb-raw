"""Scrape ESPN MBB team season stats per (season, team_id).

Output: ``mbb/team_stats/json/{season}/{team_id}.json`` -- the raw ESPN
``/teams/{team_id}/statistics?season=YYYY`` response. The downstream R
parser in ``hoopR-mbb-data`` (``espn_mbb_06_team_season_stats_creation.R``)
...
Team ids are sourced from this season's MBB schedule parquet
(``home_id`` / ``away_id``) committed by ``espn_mbb_01_schedules_scrape.py``.
...
Requirements:
    ...
    with ``league="mens-college-basketball"``.
"""
...
logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_mbb_raw_team_stats_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "mbb/team_stats/json"
...
def download_team_stats(
    ...
    try:
        raw = _espn_basketball_team_stats(
            league="mens-college-basketball", team_id=int(team_id), season=int(season), raw=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Start year of MBB team-stats scrape (YYYY)",
    )
    parser.add_argument(
        "--end_year", "--end-year", "-e", dest="end_year", type=int,
        help="End year of MBB team-stats scrape (YYYY)",
    )
    parser.add_argument(

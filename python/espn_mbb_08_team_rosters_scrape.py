"""Scrape ESPN MBB team rosters per (season, team_id).

Output: ``mbb/team_rosters/json/{season}/{team_id}.json`` -- raw ESPN
``/teams/{team_id}/roster`` response. The downstream R parser in
``hoopR-mbb-data`` (``espn_mbb_04_rosters_creation.R``) reads these JSONs to
...
logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_mbb_raw_team_rosters_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "mbb/team_rosters/json"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Start year of MBB roster scrape (YYYY)",
    )
    parser.add_argument(
        "--end_year", "--end-year", "-e", dest="end_year", type=int,
        help="End year of MBB roster scrape (YYYY)",
    )
    parser.add_argument(

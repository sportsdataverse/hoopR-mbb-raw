"""Scrape ESPN MBB athlete season stats.

Output: ``mbb/player_season_stats/json/{athlete_id}.json`` -- the raw ESPN
``/athletes/{id}/stats`` response. The downstream R parser in
``hoopR-mbb-data`` (``espn_mbb_05_player_season_stats_creation.R``) reads
...
Athlete ids are sourced from the ``espn_mens_college_basketball_player_boxscores`` release on
...
Requirements:
    Uses ``espn_mbb_player_stats`` from sportsdataverse-py
    (``sportsdataverse/mbb/mbb_player_stats.py``). The league is baked into
...
# site.web.api.espn.com/.../basketball/mens-college-basketball/athletes/{id}/stats URL.
from sportsdataverse.mbb.mbb_player_stats import espn_mbb_player_stats
from sportsdataverse.dl_utils import download


logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_mbb_raw_player_stats_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "mbb/player_season_stats/json"
PLAYER_BOX_RELEASE = (
    "https://github.com/sportsdataverse/sportsdataverse-data/releases/"
    "download/espn_mens_college_basketball_player_boxscores/player_box_{season}.parquet"
)
...
def _athlete_ids_for_season(season: int) -> list[int]:
    """Read the espn_mens_college_basketball_player_boxscores release parquet for ``season`` and
...
def download_player_stats(athlete_id: int, season: int, rerun_existing: bool) -> str:
    ...
    try:
        ...
        raw = espn_mbb_player_stats(athlete_id=int(athlete_id), season=int(season), raw=True)
...
def download_player_stats_batch(athlete_ids: list[int], season: int, rerun_existing: bool, cores: int) -> None:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for fut in tqdm(
            ...
            total=len(futs),
            desc="MBB player stats",
        ):
            fut.result()


def main() -> None:
    # ESPN MBB athlete-stats coverage tracks the box-score release, which
    # starts in 2002.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of MBB player-stats scrape (YYYY)",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of MBB player-stats scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",

...
def download_game(game, process, path_to_raw, path_to_final):
    ...
    if process == True:
        try:
            processed_data = sdv.mbb.mbb_pbp_disk(game_id=game, path_to_json=path_to_raw)

            result = sdv.mbb.helper_mbb_pbp(game_id=game, pbp_txt=processed_data)
...
def add_game_to_schedule(schedule):
    ...
    schedule["game_json_url"] = np.where(
        ...
        schedule["game_id"].apply(
            lambda x: f"https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/json/final/{x}.json"
        ),
        None,
    )
    return schedule
...
def main():
    ...
    for year in years_arr:
        schedule = pd.read_parquet(f"mbb/schedules/parquet/mbb_schedule_{year}.parquet", engine="auto", columns=None)
        ...
        if args.rescrape == False:
            schedule_in_repo = pd.read_parquet(
                "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-data/blob/main/mbb/mbb_games_in_data_repo.parquet",
                engine="auto",
                columns=None,
            )
        ...
        logger.info(f"Finished MBB PBP for {year}...")

        schedule = add_game_to_schedule(schedule)
        schedule.to_parquet(f"mbb/schedules/parquet/mbb_schedule_{year}.parquet", index=None)
        pyreadr.write_rds(f"mbb/schedules/rds/mbb_schedule_{year}.rds", schedule, compress="gzip")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of MBB Schedule period (YYYY), eg. 2023 for 2022-23 season",
    )
    parser.add_argument(
        "--end_year", "-e", type=int, help="End year of MBB Schedule period (YYYY), eg. 2023 for 2022-23 season"
    )
    parser.add_argument(

"""Scrape ESPN MBB standings per season.

Output: ``mbb/standings/json/{season}.json`` -- raw ESPN response. The
downstream R parser in ``hoopR-mbb-data`` reads these JSONs to build the
per-season tidy standings frame and upload it to the
``espn_mens_college_basketball_standings`` release tag on ``sportsdataverse-data``.
...
Requirements:
    Uses the unified cross-league ``espn_mens_college_basketball_standings`` endpoint in
...
from sportsdataverse.mbb import espn_mbb_standings
from sportsdataverse.scrape.espn.persist import write_payload


logging.basicConfig(
    level=logging.INFO,
    filename="hoopR_mbb_raw_standings_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "mbb/standings/json"
DEFAULT_THREADS = 1
...
def download_standings(
    ...
    try:
        ...
        # ESPN response dict.
        raw: dict[str, Any] = espn_mbb_standings(season=int(season))
...
def main() -> None:
    # ESPN MBB standings coverage starts in the 2002 season.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of MBB standings scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of MBB standings scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",

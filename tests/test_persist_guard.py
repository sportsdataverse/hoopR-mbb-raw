...
# Both shapes observed in committed SDV raw archives.
SPRING_ERROR = {
    "error": "Not Found",
    "message": "",
    "path": "/apis/site/v2/...",
...
@pytest.mark.parametrize(
    ...
    ids=["team_stats", "items-envelope", "player_season_stats"],
)
...
def test_an_empty_but_valid_collection_is_not_an_error():
    """A zero-row Core v2 page is a real answer. Treating it as an error
    would re-scrape it forever."""
...
@pytest.mark.parametrize(
    "pattern",
    [
        "mbb/team_stats/json/*/*.json",
        "mbb/standings/json/*.json",
        "mbb/player_season_stats/json/*.json",
    ],
)

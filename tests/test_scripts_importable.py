...
Import-time failures (a dropped dependency, a module that exists nowhere) are
exactly what a daily cron discovers at 5am and a test discovers in one second.
Ported from the wehoop-wbb-raw template pair.
"""
...
# Numbers are the ecosystem-wide ESPN-raw canon (01 schedules, 02 pbp,
# 03 standings, 04 game_rosters, 05 draft, 06 player_stats, 07 team_stats,
# 08 team_rosters, 09 player_core). 05 is an intentional hole -- no MBB draft.
EXPECTED = [
...
def test_the_scripts_are_exactly_what_we_expect():
    """A gap or a stray file means the pipeline and the directory listing
    have diverged."""
...
@pytest.mark.parametrize("path", CLI_SCRIPTS, ids=lambda p: p.name)
...
def test_no_script_uses_type_bool(path):
    ...
    assert offenders == [], (
        f"{path.name} passes type=bool at line(s) {offenders}; "
        "use sportsdataverse.scrape.espn.cli.str2bool"
    )
